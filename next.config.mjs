...
const nextConfig = {
  ...
  async headers() {
    // NOTE: X-Frame-Options and the CSP `frame-ancestors` directive are NOT set
    // here. They are owned by src/middleware.ts so they can vary per-request:
    // the admin heatmap embeds public pages as a SAME-ORIGIN iframe only under
    // ?__heatmap_bg=1, and Next.js `headers()` query-condition (has/missing)
    // rules are IGNORED at Vercel's edge (header rules match on path only).
    // Middleware runs per-request and reads the query param reliably. Every
    // other directive below applies globally.
    return [
      {
        source: "/(.*)",
        headers: [
