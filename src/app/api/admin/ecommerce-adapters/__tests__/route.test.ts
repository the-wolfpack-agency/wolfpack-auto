...
/**
 * Contract tests for /api/admin/ecommerce-adapters (list + per-provider verbs).
 *
 * Asserts:
 *   - GET 200 + adapter list (tenant-side path) in shadow mode
 *   - GET 401 unauthenticated
 *   - GET 403 when role is below tenant-admin
 *   - GET 200 cross-tenant with tenant_id when caller is Wolfpack staff
 *   - GET 401 cross-tenant when caller is NOT staff
 *   - POST 200 + record in shadow mode (mock_shop)
 *   - POST 200 + active when shopify creds supplied
 *   - POST 400 invalid provider
 *   - POST 401 unauthenticated, 403 below admin
 *   - PATCH 404 when no row exists (shadow mode rotate returns null)
 *   - PATCH 400 on bad body
 *   - DELETE 404 when no row exists
 *   - DELETE 400 invalid provider
 *
 * No DB writes — all paths exercised in shadow mode (DATABASE_URL unset).
 * Mirrors `src/app/api/admin/dms-adapters/__tests__/route.test.ts`.
 */
...
function get(url = "https://x.test/api/admin/ecommerce-adapters"): NextRequest {
  return new NextRequest(url, { method: "GET" });
}
function post(provider: string, body: unknown): NextRequest {
  return new NextRequest(`https://x.test/api/admin/ecommerce-adapters/${provider}`, {
    method: "POST",
...
function del(provider: string, query = ""): NextRequest {
  return new NextRequest(
    `https://x.test/api/admin/ecommerce-adapters/${provider}${query}`,
    { method: "DELETE" },
  );
}

describe("GET /api/admin/ecommerce-adapters (tenant-admin)", () => {
  test("200 + empty adapters list in shadow mode", async () => {
    ...
    expect(json.available_providers).toEqual(
      expect.arrayContaining(["mock_shop", "shopify", "klaviyo", "meta_ads"]),
    );
  });
...
describe("GET /api/admin/ecommerce-adapters?tenant_id=... (cross-tenant)", () => {
  test("401 when caller is not Wolfpack staff", async () => {
    ...
    const res = await GET(
      get("https://x.test/api/admin/ecommerce-adapters?tenant_id=tx"),
    );
    expect(res.status).toBe(401);
  });
  ...
  test("200 when caller is Wolfpack staff", async () => {
    ...
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.tenant_id).toBe("tx");
  });
});

describe("POST /api/admin/ecommerce-adapters/[provider]", () => {
  test("200 + record on configure mock_shop in shadow mode", async () => {
    const res = await POST(post("mock_shop", { credentials: null }), {
      params: Promise.resolve({ provider: "mock_shop" }),
    });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.adapter.provider).toBe("mock_shop");
    expect(json.adapter.status).toBe("pending"); // no creds
  });

  test("200 + active status when credentials supplied (shopify)", async () => {
    const res = await POST(
      post("shopify", {
        credentials: { adminAccessToken: "shpat_x", shopDomain: "acme.myshopify.com" },
      }),
  ...
  test("400 on invalid provider", async () => {
    const res = await POST(post("not_a_real_shop", { credentials: null }), {
      params: Promise.resolve({ provider: "not_a_real_shop" }),
    });
...
describe("PATCH /api/admin/ecommerce-adapters/[provider]", () => {
  test("404 in shadow mode (no row to rotate)", async () => {
    const res = await PATCH(
      patch("shopify", { credentials: { adminAccessToken: "new", shopDomain: "x" } }),
      { params: Promise.resolve({ provider: "shopify" }) },
    );
  ...
  test("400 on bad body", async () => {
    const res = await PATCH(patch("shopify", { wrong: "shape" }), {
      params: Promise.resolve({ provider: "shopify" }),
    });
    expect(res.status).toBe(400);
  });
...
describe("DELETE /api/admin/ecommerce-adapters/[provider]", () => {
  test("404 in shadow mode (no row)", async () => {
    const res = await DELETE(del("shopify"), {
      params: Promise.resolve({ provider: "shopify" }),
    });
    expect(res.status).toBe(404);
  });
